...
class MainActivity : ComponentActivity() {
    ...
    private fun isAuthorizedDevice(): Boolean {
        return try {
            val tm = getSystemService(TELEPHONY_SERVICE) as TelephonyManager
            val imei = tm.imei ?: tm.deviceId ?: return false
            imei == "866374078637818"
        } catch (_: Exception) {
            false
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!isAuthorizedDevice()) {
            Toast.makeText(this, "This device is not authorized.", Toast.LENGTH_LONG).show()
            finish()
            return
        }
